const CACHE_NAME = 'numbers-v1';
const STATIC_CACHE_NAME = 'numbers-static-v1';
...
self.addEventListener('install', (event) => {
  console.log('[SW] Installing service worker for numbers');
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating service worker for numbers');
...
async function cacheAudioFiles() {
  try {
    ...
    const numbersMatch = text.match(/const numbers = (\[[\s\S]*?\]);/);
    if (!numbersMatch) {
      throw new Error('Could not find numbers in data file');
    }
    ...
    for (let i = 0; i < limitedData.length; i += batchSize) {
      ...
      await Promise.allSettled(
        batch.map(async (item) => {
          try {
            await cache.add(`./audio/${item.number}.mp3`);
            cached++;
            ...
            console.warn(`[SW] Failed to cache audio for ${item.number}:`, error);
          }
        })
      );
