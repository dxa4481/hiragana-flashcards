...
const STATIC_CACHE_NAME = 'hiragana-static-v1';
...
self.addEventListener('install', (event) => {
  console.log('[SW] Installing service worker for hiragana');
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating service worker for hiragana');
...
async function cacheAudioFiles() {
  try {
    ...
    const hiraganaMatch = text.match(/const hiraganaCharacters = (\[[\s\S]*?\]);/);
    if (!hiraganaMatch) {
      throw new Error('Could not find hiragana characters in data file');
    }
